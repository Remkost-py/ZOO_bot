...
from extensions import quiz_buttons, quiz_menu_continue, quiz_menu, bot_menu, quiz_stop, review_cancel, final_message, \
    choose
...
async def send_welcome(message: types.Message):
    ...
    await bot.send_photo(photo=photo, chat_id=message.chat.id,
                         caption='Чтобы наше общение было веселым и полезным вы можете вспомнить детство и сыграть в веселую викторину !\n'
...
async def quiz_answer(callback_query: types.CallbackQuery):
    ...
    if "system" in callback_query.data:
        if "mail" in callback_query.data:
            send_mail(os.environ.get("TO_MAIL"), os.environ.get("FROM_MAIL"), os.environ.get("MAIL_PASSWORD"),
                      callback_query.from_user.username, callback_query.data.split("system.mail.")[1])
            await bot.send_message(chat_id=callback_query.message.chat.id,
                                   text="Ваш запрос был отправлен, скоро с вами свяжутся.")
        ...
        if sess.cur_qst_num < len(questions):
            ...
            button_mail = types.InlineKeyboardButton(text="Отправить письмо с предложением об опеке",
                                                     callback_data="system.mail." + totem)
            button_vk_share = types.InlineKeyboardButton(text="Поделиться ботом",
                              url="https://vk.com/share.php?url=https://t.me/Moscow_Zoo_bot")

            keyboard.add(button_link)
            keyboard.add(button_mail)
            keyboard.add(button_vk_share)
